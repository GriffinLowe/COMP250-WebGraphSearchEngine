...
public class MyWebGraph {
	...
	public boolean addVertex(String s) {
		...
		boolean contains = vertexList.containsKey(s);

		if(!contains) {
			WebVertex vertex = new WebVertex(s);
			vertexList.put(s, vertex);
		}
	...
	public boolean addEdge(String s, String t) {
		...
		// Verifying that the edge does not already exist and that the key is
		if(!vertexList.containsKey(s) || !vertexList.containsKey(t))
			return false;

		if(vertexList.get(s).links.contains(t))
			return false;

		vertexList.get(s).links.add(t);
    ...
    public ArrayList<String> getVertices() {
		...
		// Check for case where there are two URLs that are the same (is this possible???)
		ArrayList<String> vertices = new ArrayList<String>();
		for(String s: this.vertexList.keySet()) {
			vertices.add(s);
		}
    ...
    public ArrayList<String> getEdgesInto(String v) {
		...
		for(String s: this.vertexList.keySet()) {
			if(this.vertexList.get(s).links.contains(v))
				vertices.add(s);
		}
		return vertices;
    } 
